...
def _ensure_adapter_downloaded(adapter_path: str, gdrive_folder_id: str) -> bool:
    """
    Download the adapter folder from Google Drive if it doesn't exist locally.
    Returns True if the adapter is ready to use, False otherwise.
    """
    # Check if the required files are already present
    ...
    try:
        ...
        # gdown downloads folder contents into the target directory
...
def llm_worker(text_stt: multiprocessing.Queue, text_llm: multiprocessing.Queue):
    ...
    if adapter_path:
        # Resolve relative paths relative to this file's directory (voice_backend/)
        ...
        if gdrive_id:
            ...
            if not use_adapter:
                print(f"[LLM Worker] ⚠️  Adapter files not found at '{adapter_abs}' and no "
                      f"ADAPTER_GDRIVE_FOLDER_ID configured. Running base model only.")
    else:
        adapter_abs = None
    ...
    if use_adapter:
        # Prefer the fine-tuned tokenizer if it was saved alongside the adapter
    ...
    if use_adapter:
        try:
            ...
            # Merge adapter weights into base model for faster CPU inference
    ...
    # ------------------------------------------------------------------
    # 5. Custom streamer – pushes complete sentences to TTS queue
    # ------------------------------------------------------------------
    class QueueTextStreamer(TextStreamer):
        def __init__(self, tokenizer, session_id, out_queue,
                     t_stt_end, t_llm_start, **kwargs):
    ...
    while True:
        try:
            ...
            # Prompt template – tuned for the fine-tuned adapter's training format
            prompt = (
                f"Answer the following science question clearly and concisely: {text}"
            )
